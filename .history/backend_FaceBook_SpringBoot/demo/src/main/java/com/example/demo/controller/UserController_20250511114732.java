...
public class UserController {
    ...
    @Operation(summary = "Create user with avatar", description = "Create a new user with avatar image upload")
    ...
    public ResponseEntity<UserDto> createUserWithAvatar(
            @RequestPart(value = "data", required = true) @Valid UserCreateRequest request,
            @RequestPart(value = "file", required = false)
            @Parameter(description = "Avatar image file (PNG, JPG, JPEG)", content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE))
            MultipartFile file) {
        try {
